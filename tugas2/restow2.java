...
public class restow2 {
	...
	private static void searchJalan(Queue<String> jalan, int r, int c){
		...
		stc.push(r + "," + c);
		while(!stc.isEmpty()){
			String[] pisahkan = stc.pop().split(",");
			int x = Integer.parseInt(pisahkan[0]);
			int y = Integer.parseInt(pisahkan[1]);
			if(!visit.contains(x + "," + y)){
				visit.add(x + "," + y);
				jalan.add(x + "," + y);
				for(int i = 0; i < 4; i++){
					if(getMap(x+idxA[i],y+idxB[i]) != '#'){
						stc.push((x+idxA[i]) + "," + (y+idxB[i]));
					}
				}
				setMap(x,y,'#');
			}
